...
public class VippsAppGuiScreen extends AbstractContainerScreen<VippsAppGuiMenu> implements ItacraftModScreens.ScreenAccessor {
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.drawString(this.font, Component.literal("Real Vipps"), 40, 7, -12829636, false);
        
        // Get the phone from player's hand
        ItemStack phoneItem = this.entity.getMainHandItem();
        
        // Get money value from phone's NBT
        double money = phoneItem.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY)
            .copyTag()
            .getDouble("money")
            .orElse(0.0);
        
        // Format and display the money
        String moneyText = String.format("%.2f kr", money);
        
        guiGraphics.drawString(this.font, Component.literal(moneyText), 34, 103, -12829636, false);
    }
    ...
    public void init() {
        ...
        // Close button at the bottom - just closes the GUI directly
        this.addRenderableWidget(Button.builder(Component.literal("o"), e -> {
            this.minecraft.player.closeContainer();
        }).bounds(this.leftPos + 65, this.topPos + 205, 20, 20).build());
    }
}